	...
	#' @param return_response Whether to return the response or the response content. Defaults to FALSE (return response content).
	#' @export
	domains.insert <- function(customer, domainAliases = NULL, verified = NULL, isPrimary = NULL, etag = NULL, kind = NULL, creationTime = NULL, domainName = NULL, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.domains.insert']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
