	...
	#' @export
	asps.delete <- function(userKey, codeId, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.asps.delete']]$scopes), return_response = F){
	...
	#' @param return_response Whether to return the response or the response content. Defaults to FALSE (return response content).
	#' @export
	asps.get <- function(userKey, codeId, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.asps.get']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
