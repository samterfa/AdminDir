	...
	#' @param return_response Whether to return the response or the response content. Defaults to FALSE (return response content).
	#' @export
	mobiledevices.list <- function(customerId, pageToken = NULL, maxResults = NULL, orderBy = NULL, sortOrder = NULL, projection = NULL, query = NULL, gargle_token = gargle::token_fetch(scopes = .endpoints[['directory.mobiledevices.list']]$scopes), return_response = F){
		params <- as.list(environment())[!names(as.list(environment())) %in% c('return_response', 'gargle_token')]
